...
class McpController {
	...
	public static function discover_abilities( $request ) {
		...
		if ( function_exists( 'wp_get_abilities' ) ) {
			...
			foreach ( Registry::get_fallback_abilities() as $ability ) {
				if ( ! empty( $ability_names ) && ! in_array( $ability['name'], $ability_names, true ) ) {
					continue;
				}
	...
	public static function get_ability_info( $request ) {
		...
		$fallback_ability = Registry::get_fallback_ability( $ability_name );
		if ( $fallback_ability ) {
			return array( 'ability' => $fallback_ability );
		}
	...
	public static function execute_ability( $request ) {
		...
		$result = Registry::execute_fallback_ability( $ability_name, $input );
		if ( null === $result ) {
			return self::not_found_error( $ability_name );
		}
	 ...
	 * @return WP_Error
	 */
	private static function not_found_error( $ability_name ) {
		return new WP_Error(
