...
"Auftrag" (stored key `Vertrag`). Both dates use the current Berlin calendar date
...
- Other results, including Stattgefunden + Auftrag, retain their existing behavior.
- No historical Lead data is backfilled and no customer or finance document is created.
