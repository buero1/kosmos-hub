...
class SiteRepository:
    ...
    def get_by_uuid(self, site_uuid: str) -> Site | None:
        statement = (
            ...
            .options(
                ...
                selectinload(Site.user_snapshots),
            )
    ...
    def get_latest_site_user_snapshot(self, site_id: int) -> SiteUserSnapshot | None:
        statement = (
            select(SiteUserSnapshot)
            .where(SiteUserSnapshot.site_id == site_id)
            .order_by(SiteUserSnapshot.captured_at.desc())
            .limit(1)
        )
        return self.db.scalar(statement)
    ...
    def get_latest_user_snapshots_by_site_ids(self, site_ids: list[int]) -> dict[int, SiteUserSnapshot]:
        if not site_ids:
            return {}

        latest_captured_at = (
            select(
                SiteUserSnapshot.site_id,
                func.max(SiteUserSnapshot.captured_at).label("captured_at"),
            )
            .where(SiteUserSnapshot.site_id.in_(site_ids))
            .group_by(SiteUserSnapshot.site_id)
            .subquery()
        )
        statement = select(SiteUserSnapshot).join(
            latest_captured_at,
            (SiteUserSnapshot.site_id == latest_captured_at.c.site_id)
            & (SiteUserSnapshot.captured_at == latest_captured_at.c.captured_at),
        )
        snapshots = self.db.scalars(statement).all()
        return {snapshot.site_id: snapshot for snapshot in snapshots}
    ...
    def create_site_user_snapshot(
        ...
        *,
        ...
        snapshot = SiteUserSnapshot(
            ...
            encrypted_users_json=encrypted_users_json,
            message=message,
        )
        self.db.add(snapshot)
        self.db.flush()
