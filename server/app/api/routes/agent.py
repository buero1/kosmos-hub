...
from app.services.hub_agent_files import HubAgentFileError, MAX_AGENT_FILE_BYTES, extract_agent_file
from app.services.hub_finance_pdf_generation import run_finance_pdf_generation
...
def agent_page(
    ...
    return templates.TemplateResponse(
        ...
        "agent.html",
        {
            ...
            "ai_usage": HubOperationService(db=db, cipher=cipher, actor=user.username).query("ai.usage.read", {"detail": "recent"}),
...
def save_email_ai_prompt_presets(
    ...
    service = HubAdministrationService(db=db, cipher=get_secret_cipher(), actor=user.username)
    enabled_preset_ids = set(enabled_preset_id)
    try:
        service.save_prompt_presets(
            presets=tuple(
                EmailAiPromptPresetInput(
            ...
            new_label=new_label, new_instruction=new_instruction,
        )
        write_audit_log(
            db,
            site=None,
...
async def upload_agent_file(
    ...
    try:
        ...
        extraction = await run_in_threadpool(extract_agent_file, filename=file.filename or "", content=content)
        chat = HubAgentService(db=db, cipher=get_secret_cipher()).add_file_context(
            actor=user.username,
            conversation_id=_required_id(conversation_id),
            extraction=extraction,
        )
        write_audit_log(
            db, site=None, actor=user.username, source="hub-agent",
            action="agent-file-context-added", result="success",
            detail=f"Added file context to Hub agent conversation {chat.conversation_id}; file content omitted.",
        )
        db.commit()
    except (HubAgentError, HubAgentFileError) as exc:
        db.rollback()
        return _chat_error(str(exc), status_code=400)
    finally:
        await file.close()
...
def execute_agent_chat_action(
    ...
    try:
        ...
        db.commit()
        if action.status == "completed" and action.background_token:
            background_tasks.add_task(run_finance_pdf_generation, action.background_token)
    except HubAgentError as exc:
        db.rollback()
        return _chat_error(str(exc), status_code=400)
    return JSONResponse(_chat_payload(request, chat))
        ...
        "messages": [
            {
                ...
                "ocr_context_used": job.ocr_context_used,
                "status": job.status,
                "created_at": iso_berlin_time(job.created_at),
                "actions": [
                    {
                        "id": action.id,
        ...
        "conversations": [
            {
                ...
                "updated_at": iso_berlin_time(conversation.updated_at),
